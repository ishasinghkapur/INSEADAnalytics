...
Some finance data loading tools
Connecting directly to Bloomberg
Some time series plot tools
Various finance code links
More links
Even more links
Of course endless available code (e.g. like this one that seems to get companies' earnings calendars)
Optional Question

Can you find some interesting finance related R package or github repository? Your Answers here: 
...
The cumulative returns of the S&P index during this period is r round(100*sum(StockReturns[,1]),1)%.
The average daily returns of the S&P index during this period is r round(100*mean(StockReturns[,1]),3)%;
...
Notice that the code also downloads the returns of Apple during the same period. Can you explain where this is done in the code (including the .R files used)?
What are the cumulative, average daily returns, and the standard deviation of the daily returns of Apple in the same period?
(Extra points) What if we want to also see the returns of another company, say Yahoo!, in the same period? Can you get that data and report the statistics for Yahoo!'s stock, too?
Your Answers here: 
1. In the dataset1.R file


2. The cumulative returns of Apple during this period is r round(100*sum(StockReturns[,2]),1)%. The average daily returns of Apple during this period is r round(100*mean(StockReturns[,2]),3)%; The standard deviation of the daily returns of Apple during this period is r round(100*sd(StockReturns[,2]),3)%;


3. Using the ticker YHOO and the same code in dataSet1.R, I duplicated it to dataSet2.R. The result is -

The cumulative returns of Yahoo during this period is r round(100*sum(StockReturns_2[,3]),1)%. The average daily returns of Yahoo during this period is r round(100*mean(StockReturns_2[,3]),3)%; The standard deviation of the daily returns of Yahoo during this period is r round(100*sd(StockReturns_2[,3]),3)%; 
...
What R commands can you use to get the number of rows and number of columns of the new matrix called transposedData?
Based on the help for the R function apply (help(apply)), can you create again the portfolio of S&P and Apple and plot the returns in a new figure below?
Your Answers here: 
1. Rows = nrow(t(StockReturns)) ncol = (t(StockReturns))
...
Your Answers here: 
1. Yes, in the R script - dataSet1.R, in the code "startDate" 

2. The following is a portfolio of S&P, Apple, Yahoo, Goldman Sachs, and Microsoft

portfolio_2 = apply(StockReturns_2,1,mean)
names(portfolio_2) <- rownames(StockReturns_2)
...
Once you write and read the data as described above, what happens when you run this command in the console of the RStudio: sum(myData != StockReturns[1:20,])
(Extra exercise) What do you think will happen if you now run this command, and why:
myData + StockReturns[1:40,]
Your Answers here: 
1. It returns the figure 20. 
